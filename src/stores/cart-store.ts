      ...
      persist(
        (set, get) => ({
              ...
              if (existingStoreIndex >= 0) {
                ...
                if (existingItemIndex >= 0) {
                  ...
                  updatedStores[existingStoreIndex].subtotal = updatedStores[existingStoreIndex].items.reduce(
                    (total, cartItem) => total + (cartItem.sellingPrice * cartItem.quantity), 0
                  )
                ...
                const newStore: CartStore = {
                  ...
                  storeName: item.storeName || '',
                  items: [newItem],
                  subtotal: newItem.sellingPrice * quantity,
                }
            ...
            try {
              ...
              if (existingStoreIndex >= 0) {
                ...
                const newStore: CartStore = {
                  ...
                  storeName: bundle.storeName || '',
                  items: [newBundle],
                  subtotal: newBundle.sellingPrice * quantity,
                }
          ...
          removeItem: (itemId, storeId) => {
            try {
              const { cart } = get()
              const storeIndex = cart.stores.findIndex((store) => store.storeId === storeId)
              
              if (storeIndex >= 0) {
                const updatedStores = [...cart.stores]
                const updatedItems = updatedStores[storeIndex].items.filter((item) => item.id !== itemId)
                
                if (updatedItems.length === 0) {
                  // If no items left in store, remove the store
                  updatedStores.splice(storeIndex, 1)
                } else {
                  // Update store with remaining items
                  updatedStores[storeIndex] = {
                    ...updatedStores[storeIndex],
                    items: updatedItems,
                    subtotal: updatedItems.reduce((total, item) => total + (item.sellingPrice * item.quantity), 0)
                  }
                }
            ...
            try {
              ...
              if (storeIndex >= 0) {
                ...
                if (itemIndex >= 0) {
                  if (quantity > 0) {
                    // Update quantity
                    updatedStores[storeIndex].items[itemIndex] = {
                      ...updatedStores[storeIndex].items[itemIndex],
                      quantity
                    }
                    
                    // Recalculate store subtotal
                    updatedStores[storeIndex].subtotal = updatedStores[storeIndex].items.reduce(
        ...
        }),
        {
          name: 'shopping-cart',
          version: 2,
          partialize: (state) => ({ cart: validateCartState(state.cart) }),
          onRehydrateStorage: () => (state) => {
            if (state) {
              state.cart = validateCartState(state.cart)
            }
          },
        }
      )
